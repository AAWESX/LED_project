...
uint32_t BACK_COLOR =0x00000000; //背景色
...
//(y) :横轴坐标
//(x1 ,x2): 纵轴虚线长度
void LCD_HoriDashed(uint16_t y ,uint16_t x1, uint16_t x2)
{
    uint16_t j;
    for(uint16_t i = x1 ; i < x2 ;i+=8)
    {
        j = i + 2;
        LCD_DrawLine(i ,y,j,y);
    }
}

//画横虚线排列
void LCD_ArrangedHoriDashed(uint16_t y1, uint16_t y2 ,uint16_t x1, uint16_t x2)
{
    uint16_t ggz = (y2 + y1)/2;
    uint16_t num = 0;
    for(uint16_t i = ggz; i < y2 ; i+=35)
    {
        num++;
        uint16_t j;
        for(uint16_t i2 = x1 ; i2 < x2 ;i2+=8)
        {
            j = i2 + 2;
            LCD_DrawLine(i2 ,i,j,i);
            if(ggz - (35*num) > 0)
            {
                LCD_DrawLine(i2 ,ggz - (35*num),j,ggz - (35*num));
            }
...
//(x) :纵轴坐标
//(y1 ,y2): 横轴虚线长度
void LCD_Dashed(uint16_t x ,uint16_t y1, uint16_t y2)
{
    uint16_t j;
    for(uint16_t i = y1 ; i < y2 ;i+=8)
    {
        j = i + 2;
        LCD_DrawLine(x, i, x, j);
    }
}                          

//画纵虚线排列
void LCD_ArrangedDashed(uint16_t x1 , uint16_t x2 ,uint16_t y1, uint16_t y2)
{
    uint16_t num = 0;
    uint16_t ggz = (x2 + x1)/2;
    for(uint16_t i = ggz; i < x2 ; i+=35)
    {
        num++;
        uint16_t j;
        for(uint16_t i2 = y1 ; i2 < y2 ;i2+=8)
        {
            j = i2 + 2;
            LCD_DrawLine(i, i2, i, j);
            if(ggz - (num*35) > x1)
            {
                LCD_DrawLine(ggz - (num*35), i2, ggz - (num*35), j);
            }
        }
...
//(x1,x2):十字形横线
//(y1,y2):十字形竖线
void LCD_CrossShape(uint16_t x1, uint16_t x2 ,uint16_t y1 ,uint16_t y2)
{
    LCD_DrawLine(0 ,  x1 , 700 , x2);
    LCD_DrawLine(y1,20, y2 , 450);
}
...
//画矩形
//(x1,y1),(x2,y2):矩形的对角坐标
void LCD_DrawSolidRectangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t *color)
{
    POINT_COLOR=*color;
    LCD_DrawLine(x1, y1, x2, y1);
    LCD_DrawLine(x1, y1, x1, y2);
...
//(x,y):坐标
//r    :半径
void LCD_Draw_ALLCircle(uint16_t x0,uint16_t y0,uint8_t r)
{
	int a,b;
	int di;
	a=0;b=r;	  
